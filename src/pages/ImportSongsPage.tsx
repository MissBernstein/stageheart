...
// Full CSV data with all 129 songs
const FULL_SONGS_CSV = `title,artist,summary,theme,core_feelings,access_ideas,visual,version_label,source,locale
...
export const ImportSongsPage: React.FC = () => {
  ...
  const runImport = async () => {
    setImporting(true);
    setResult(null);
    setLogs(['Starting client-side import of 129 songs...']);

    try {
      let success = 0;
      let skipped = 0;
      let errors = 0;
      const logs: string[] = ['Parsing CSV data...'];
      
      // Parse all songs from embedded CSV data
      const songsData = parseCsvData(FULL_SONGS_CSV);
      ...
      for (const song of songsData) {
        ...
        // Update logs every 10 songs to show progress
        if ((success + skipped + errors) % 10 === 0) {
          setLogs([...logs]);
        }
      }
